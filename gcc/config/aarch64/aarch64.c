...
aarch64_pad_arg_upward (enum machine_mode mode, const_tree type)
{
  ...
  if (type
      ? (INTEGRAL_TYPE_P (type) || SCALAR_FLOAT_TYPE_P (type) || POINTER_TYPE_P (type))
...
aarch64_frame_pointer_required (void)
{
  ...
  if (flag_omit_frame_pointer && !faked_omit_frame_pointer)
    ...
    return !current_function_is_leaf || df_regs_ever_live_p (LR_REGNUM);
  return true;
}
...
aarch64_trampoline_init (rtx m_tramp, tree fndecl, rtx chain_value)
{
  ...
  emit_library_call (gen_rtx_SYMBOL_REF (Pmode, "__clear_cache"),
		     ...
		     plus_constant (a_tramp, TRAMPOLINE_SIZE),
		     ptr_mode);
}
