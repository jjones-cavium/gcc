  ...
  for (i = 0; i < n_sets; i++)
	...
	src_folded = src_const, src_folded_cost = src_folded_regcost = -1;
	 ...
	 (set (zero_extract:M2 (reg:M N) (const_int C) (const_int D))
	      (reg:M2 O)).
	 This is done without respect of the costs of reg O vs the cost of the constant B.  */
      if (elt)
	{
	  struct table_elt *elt1 = elt;
	  for (elt1 = elt; elt1; elt1 = elt1->next_same_value)
	    {
	      rtx trial;
	      if (!elt1->is_const || !CONST_INT_P (elt1->exp))
		continue;
	      trial = elt1->exp;
	      if (GET_CODE (SET_DEST (sets[i].rtl)) == ZERO_EXTRACT
		  && CONST_INT_P (trial)
